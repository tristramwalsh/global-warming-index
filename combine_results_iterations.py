...
def combine_repeats(result_type, regressed_years, regressed_vars):
    dict_iterations = {}
    size_iterations = {}

    iterations = [f.split('DATE-CALCULATED--')[-1].split('.')[0]
                  for f in os.listdir(results_folder)
                  if result_type in f
                  and f"VARIABLES--{regressed_vars}" in f
...
results_dfs = {}
if re_calculate:
    # TODO: change to separate out the regressed variables and regressed years
    # to be specific to each other in a nested dictionary, as I often won't
    # be calculating the same years for each set of variables. At the moment
    # this just finds all years, and all variables, and loops over them, but
    # this leads to issues of trying to combine files that don't exist.
    regressed_variables_all = sorted(list(set([
        f.split('VARIABLES--')[-1].split('_')[0] 
        for f in os.listdir(results_folder)
        if 'AVERAGE' not in f
        and 'HISTORICAL-ONLY' not in f])))

    print('Regressed variables all:', regressed_variables_all)

    for regressed_vars in regressed_variables_all:
        regressed_years_vars = sorted(list(set([
            f.split('REGRESSED-YEARS--')[-1].split('_')[0]
            for f in os.listdir(results_folder)
            if regressed_vars in f
            if 'AVERAGE' not in f
            and 'HISTORICAL-ONLY' not in f])))
        print('Regressed years vars:', regressed_years_vars)

        for regressed_years in regressed_years_vars:
            # Timeseries averaging
            df_avg, dict_iterations, size_iterations = combine_repeats(
                'timeseries', regressed_years, regressed_vars)
            # Headlines may not have been calculated if only timeseries were
            # needed
            if headline_toggle:
                combine_repeats('headlines', regressed_years, regressed_vars)
...
for f in os.listdir(results_folder):
    if 'AVERAGE' in f:
        var = f.split('VARIABLES--')[-1].split('_')[0]
        reg_years = f.split('REGRESSED-YEARS--')[-1].split('_')[0]
        if var not in results_files.keys():
            results_files[var] = {reg_years: f'{results_folder}/{f}'}
        else:
            results_files[var][reg_years] = f'{results_folder}/{f}'

print('Loading all averaged datasets')
for reg_vars in results_files.keys():
    for reg_years in results_files[reg_vars].keys():
        df_iteration = pd.read_csv(
                results_files[reg_vars][reg_years],
                index_col=0,  header=[0, 1], skiprows=0)
        if reg_vars not in results_dfs.keys():
            results_dfs[reg_vars] = {reg_years: df_iteration}
        else:
            results_dfs[reg_vars][reg_years] = df_iteration
...
for reg_vars in sorted(results_dfs.keys()):
    ...
    for reg_years in reg_years_all:
        ...
        plot_vars = results_dfs[
            reg_vars][reg_years].columns.get_level_values(0).unique().to_list()
        ...
        gr.gwi_timeseries(
            ax, df_temp_Obs, None,
            results_dfs[
            reg_vars][reg_years].loc[reg_end:, :],
        ...
        gr.gwi_timeseries(
            ax, df_temp_Obs, None,
            results_dfs[
            reg_vars][reg_years].loc[reg_start:reg_end, :],
            plot_vars, var_colours)
...
for reg_vars in sorted(results_dfs.keys()):
    ...
    df_hist = results_dfs[reg_vars][reg_years_all[0]].copy()
    df_hist[:] = 0

    # For each iteration, add the row that corresponds to the final year of the
    # regressed years to the new df_hist. The row indedx it should be inserted
    # at is the same as the second year in the iteration name.
    for reg_years in reg_years_all:
        df_hist.loc[int(reg_years.split('-')[1])] = \
            results_dfs[reg_vars][reg_years].loc[int(reg_years.split('-')[1])]
    # Remove all years that are not the end of an attribution period to avoid
    # confusion (i.e. the longer earlier years before the historical-only focus
    # period).
    end_years = [int(reg_years.split('-')[1]) for reg_years in reg_years_all]
    smallest_end_year = min(end_years)
    largest_end_year = max(end_years)

    df_hist = df_hist.loc[smallest_end_year:, :]
    results_dfs[reg_vars]['HISTORICAL-ONLY'] = df_hist
    df_hist.to_csv(
    ...
    print('Plotting historical-only vs full dataset for:', reg_vars)
    plot_vars = df_hist.columns.get_level_values(0).unique().to_list()
    fig = plt.figure(figsize=(12, 8))
    ax = plt.subplot2grid(shape=(1, 1), loc=(0, 0), rowspan=1, colspan=1)
    gr.gwi_timeseries(
        ax, df_temp_Obs, None,
        results_dfs[reg_vars]['HISTORICAL-ONLY'],
        plot_vars, var_colours, sigmas=['5', '95', '50'],
        hatch='\\', linestyle='dashed')
    gr.gwi_timeseries(
        ax, df_temp_Obs, None,
        results_dfs[reg_vars][max_regressed_range],
        plot_vars, var_colours, sigmas=['5', '95', '50'],
    ...
    # Calculate how the expected final year of the timeseries changes depending
    # on the years that are regressed. Expect that the attributed values in
    # 2023 (end year of the full timeseries) will have larger uncertainties,
    # the earlier/shorter the range of regressed years is.

    # Create new empty dataframes to store the constrained results:
    constrained_year = end_yr
    df_constrained = results_dfs[reg_vars][reg_years_all[0]].copy()
    df_constrained[:] = 0

    # For each iterstion, add the final row of the dataframe to the new
    # df_hist. The row index it should be inserted at is the same as the second
    # year in the iteration name.
    for reg_years in reg_years_all:
        # print(iteration, iteration.split('-')[1], constrained_year)
        df_constrained.loc[int(reg_years.split('-')[1])] = \
            results_dfs[reg_vars][reg_years].loc[constrained_year]
    ...
    fig.savefig(
        f'plots/Projected_warming_in_{constrained_year}_' +
    ...
    # dataset
    # 2. the change in temp in year Y+1 in the old dataset to the temp in year
    ...
    # Only factor 1 and 2 are considered in this calcualtion. The other factors
    # may be added later, but sourcing historical T and ERF data is
    # significantly more wrangling.

    print('Creating delta contributions for:', reg_vars)
    ...
    for y in differ_years:
        delta_new = (results_dfs[reg_vars][f'1850-{y}'].loc[int(y)] -
                     results_dfs[reg_vars][f'1850-{y}'].loc[int(y)-1])
        delta_rev = (results_dfs[reg_vars][f'1850-{y}'].loc[int(y)-1] -
                     results_dfs[reg_vars][f'1850-{int(y)-1}'].loc[int(y)-1])
        df_delta_revised_previous_year.loc[int(y)] = delta_rev
        df_delta_additional_forcing_year.loc[int(y)] = delta_new

    # Plot the results
    # The red line is the additional warming in year Y+1 relative to year Y in
    # the new dataset.
    ...
    # dataset for year Y. That is so say, this line comes from the
    ...
    for year in years:
        df_new = results_dfs[reg_vars][f'1850-{year}']
        ...
        if year == max(years):
            ...
            df_old = results_dfs[reg_vars][f'1850-{year-1}']
